...
def aircanvas(request):
    ...
    while True:
        ...
        if len(lmList) > 0:
            ...
            xp, yp = x1, y1  # Update the previous point to the current point
        else:
            xp, yp = 0, 0 
        ...
        if y1 < 50:
            selected_color_index = int(x1 / (width / len(color_dict)))
            if selected_color_index < len(color_dict):
                drawColor = list(color_dict.values())[selected_color_index]
